...
{cal_cell_content}
    ...
    <div class="content">{content}</div> <!-- Display content here -->
{/cal_cell_content}
    ...
    public function get_calender_data($year, $month)
{
    ...
    if ($query === FALSE) {
        // Handle query error here, such as logging or returning an empty array
        return array();
    }

    $cal_data = array();
    foreach ($query->result() as $row) {
        $calendar_date = date("j", strtotime($row->tanggal));
        $cal_data[(int)$calendar_date] = $row->isi;
    }
